...
If you have the file `LandscapeFile.pdf` with 5 pages total, starting at page 2 (e.g. the first page is a cover), and the last page is a single page, so already portrait:

```
python3 poster.py -i LandscapeFile.pdf -o PortraitFile.pdf -n 5 -s 2 -e
```

## Examples

Suppose your input.pdf looks like `[C1, P2P3, P4P5, P6]` (`C1` is a cover page). Run `python3 poster.py -i input.pdf -o output.pdf -s 2 -n 4 -e`. Your output will be `[P2, P3, P4, P5, P6]`.
...
**-n**: The last page to extract from the input (one-indexed).
